...
> **Cache miss(캐시 미스)**
>
> &nbsp;**컴퓨터에서 사용되는 _Cache memory (캐시 메모리)_ 에서 필요한 데이터가 캐시에 없는 상태를 말한다.**
> 이 경우 CPU는 데이터를 찾기 위해 _Main memory (메인 메모리)_ 나 _Hard disk (하드 디스크)_ 에 접근해야 한다.
>
> &nbsp;_Cache memory_ 는 CPU가 필요로 하는 데이터를 미리 캐시에 저장해두어 빠른 처리를 가능하게 한다.
> 하지만 <ins>캐시에 저장된 데이터가 최신 데이터가 아닐 경우, 혹은 해당 데이터가 캐시에 저장되어 있지 않을 경우 _Cache miss_ 가 발생한다.</ins>
> 이 경우 CPU는 캐시에서 데이터를 찾을 수 없으므로 _Main memory_ 나 _Hard disk_ 에 접근해야 하므로 처리 속도가 느려진다.
>
> &nbsp;_Cache miss_ 가 발생하는 이유는 크게 두 가지로 나뉜다. <ins>첫 번째는 _Cache memory_ 의 크기가 작을 경우, 두 번째는 캐시 교체 알고리즘의 문제 등이 있다.</ins>
> _Cache memory_ 크기가 작을 경우 적은 양의 데이터만 저장할 수 있기 때문에 _Cache miss_ 가 발생할 가능성이 높다.
> 또한, 캐시 교체 알고리즘이 불필요하게 _Cache memory_ 의 데이터를 지우는 경우에도 _Cache miss_ 가 발생할 수 있다.
>
> &nbsp;_Cache miss_ 를 최소화하기 위해서는 적절한 _Cache memory_ 의 크기와 캐시 교체 알고리즘을 선택하는 것이 중요하다.
